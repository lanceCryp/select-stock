...
if result:
    results.append({'name': 'PP>0.25基准', **result})
    print(f"  PP>0.25: 年化 {result['annual']*100:+.1f}% | 夏普 {result['sharpe']:.2f} | 胜率 {result['win_rate']*100:.0f}%")

# 1. PP阈值精细化
print("\n1. PP阈值精细化 (0.22-0.30)")
for pp in np.arange(0.22, 0.32, 0.01):
...
for top_n in [3, 4, 5, 6]:
    ...
    if result:
        results.append({'name': f'PP>0.25+持仓{top_n}只', **result})
...
for hold_days in [45, 50, 55, 60, 65]:
    ...
    if result:
        results.append({'name': f'PP>0.25+{hold_days}天', **result})
        print(f"  {hold_days}天: 年化 {result['annual']*100:+.1f}% | 夏普 {result['sharpe']:.2f} | 胜率 {result['win_rate']*100:.0f}%")

# 4. 止损
print("\n4. 止损优化")
for stop_loss in [0.05, 0.08, 0.10, 0.12, 0.15]:
    rd = val_dates[::55]
    result = backtest(val_df, rd, make_func(conditions), top_n=4, stop_loss=stop_loss)
    if result:
        results.append({'name': f'PP>0.25+止损{stop_loss*100:.0f}%', **result})
...
for rsi in [50, 55, 60, 65, 70]:
    new_conditions = [('price_position', '>', 0.25), ('rsi', '<', rsi)]
    def make_func2(cond):
        def func(d, n):
            mask = apply_rules(d, cond)
            filtered = d[mask]
            ...
            return d.nlargest(n, 'lgb_pred')
        return func
    result = backtest(val_df, val_dates[::55], make_func2(new_conditions), top_n=4)
    if result:
        results.append({'name': f'PP>0.25+RSI<{rsi}', **result})
        print(f"  RSI<{rsi}: 年化 {result['annual']*100:+.1f}% | 夏普 {result['sharpe']:.2f} | 胜率 {result['win_rate']*100:.0f}%")
...
print(f"\n✅ 最终策略已保存: output/best_strategy_final.json")

# 保存所有结果
results_df = pd.DataFrame([{k: v for k, v in r.items() if k != 'portfolio'} for r in results])
results_df.to_csv('output/all_strategies.csv', index=False, encoding='utf-8-sig')
print(f"✅ 所有策略已保存: output/all_strategies.csv")