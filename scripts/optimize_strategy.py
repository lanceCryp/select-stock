...
print(f"数据: {df['code'].nunique()} 只股票, {df['date'].min().date()} ~ {df['date'].max().date()}")
...
def backtest(data, dates, strategy_func, top_n=20, stop_loss=0):
    """回测函数"""
    cash = INITIAL_CASH
    positions = {}
    portfolio_values = []
...
# 1. 纯 LGB 不同参数
print("\n测试1: 纯 LGB 不同持仓周期")
for hold_days in [10, 20, 30, 40]:
    rebal_dates = val_dates[::hold_days]
    strategy = lambda d, n: d.nlargest(n, 'lgb_pred')
    result = backtest(val_df, rebal_dates, strategy, top_n=20)
    if result:
        results.append({
            'name': f'LGB 持仓{hold_days}天',
            'hold_days': hold_days,
            'type': 'LGB',
            **result
        })
        print(f"  持仓{hold_days}天: 年化 {result['annual']*100:+.1f}% | 夏普 {result['sharpe']:.2f} | 胜率 {result['win_rate']*100:.0f}%")
...
for top_n in [5, 10, 20, 30, 50]:
    ...
    if result:
        results.append({
            'name': f'LGB 持仓{top_n}只',
            'top_n': top_n,
            'type': 'LGB',
            **result
        })
        print(f"  持仓{top_n}只: 年化 {result['annual']*100:+.1f}% | 夏普 {result['sharpe']:.2f} | 胜率 {result['win_rate']*100:.0f}%")

# 3. LGB + 止损
print("\n测试3: LGB + 止损")
for stop_loss in [0.05, 0.10, 0.15, 0.20]:
    strategy = lambda d, n: d.nlargest(n, 'lgb_pred')
    result = backtest(val_df, rebal_dates, strategy, top_n=20, stop_loss=stop_loss)
    if result:
        results.append({
            'name': f'LGB 止损{stop_loss*100:.0f}%',
            'stop_loss': stop_loss,
            'type': '止损',
            **result
        })
        print(f"  止损{stop_loss*100:.0f}%: 年化 {result['annual']*100:+.1f}% | 夏普 {result['sharpe']:.2f} | 胜率 {result['win_rate']*100:.0f}%")

# 4. 规则策略
print("\n测试4: 规则策略")
conditions_list = [
    [('price_position', '<', 0.5), ('rsi', '<', 50)],
    [('price_position', '<', 0.4), ('rsi', '<', 40)],
    [('price_position', '<', 0.3)],
    [('rsi', '<', 30)],
    [('volatility', '>', 0.02)],  # 高波动
]
...
for conditions in conditions_list:
    ...
    result = backtest(val_df, rebal_dates, make_func(conditions), top_n=20)
    if result:
        cond_str = '&'.join([f"{c[0]}{c[1]}{c[2]}" for c in conditions])
        results.append({
            'name': f'规则:{cond_str[:20]}',
            'conditions': conditions,
            'type': '规则',
            **result
        })
        print(f"  规则 {cond_str[:25]}: 年化 {result['annual']*100:+.1f}% | 夏普 {result['sharpe']:.2f} | 胜率 {result['win_rate']*100:.0f}%")

# 5. 动量 vs 反转
print("\n测试5: 动量 vs 反转")
for ret_col in ['ret_20d', 'ret_5d']:
    for asc in [True, False]:  # True=反转（跌得多）, False=动量（涨得多）
        strategy = lambda d, n, col=ret_col, a=asc: d.sort_values(col, ascending=a).head(n)
        result = backtest(val_df, rebal_dates, strategy, top_n=20)
        if result:
            direction = '反转' if asc else '动量'
            results.append({
                'name': f'{direction} {ret_col}',
                'type': '动量反转',
                **result
            })
            print(f"  {direction} {ret_col}: 年化 {result['annual']*100:+.1f}% | 夏普 {result['sharpe']:.2f} | 胜率 {result['win_rate']*100:.0f}%")
...
# 保存
with open('output/optimize_result.json', 'w') as f:
    json.dump({
...
print("\n结果已保存: output/optimize_result.json")